...
### CURSOR HANDOFF
Use Cursor for routine implementation. Before writing code, ask: **Does Claude actually need to implement this?** If not, STOP and output `CURSOR HANDOFF`, then provide a ready-to-paste Cursor prompt with objective, context, relevant files, requirements, constraints, acceptance criteria, tests, and verification commands. Do not continue implementing unless explicitly told to stay in Claude. When uncertain, prefer Cursor.

### CLAUDE
Reserve Claude for architecture, system design, difficult reasoning, ambiguous requirements, complex debugging, security-sensitive decisions, tradeoffs, planning, task decomposition, review, and integration. Once the hard reasoning is solved, hand straightforward implementation to Cursor.

### RINGER
Use Ringer when available and parallel execution provides meaningful advantage. Claude owns architecture, decomposition, acceptance criteria, integration, and final review. Every Ringer task needs executable verification. If Ringer is unavailable, output `RINGER RECOMMENDED` with the task breakdown.

### VERIFICATION
Never trust an AI worker completion claim by itself. Run relevant type checks, lint, tests, build, and end-to-end checks. Never weaken tests just to make them pass.

### PRIORITY
Implementation: **Cursor first → Ringer for meaningful parallelism → Claude only when genuinely necessary.**
Reasoning: **Claude first → Cursor/Ringer for implementation → Claude final review.**
<!-- AI-ROUTING-END -->
